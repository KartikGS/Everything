...
> This document is project-agnostic. It defines the universal feedback and discrepancy-resolution model for multi-agent systems. For project-specific file paths, see your project's context extension.
...
If an agent (Analyst, Lead, or Sub-Agent) identifies one of the following, they MUST NOT silently fix it if it belongs to another role's authority:

- **Missing Contract**: A component lacks an expected property (e.g., a missing `id` required for testing).
- **False Assumption**: A requirement or plan is based on a technical premise that is found to be incorrect.
...
- **Action**:
    ...
    2. Document the issue in the role's report file under a `## BLOCKER / FEEDBACK` section.
    3. Clearly state:
        - What was expected (per plan).
        - What was found (reality).
        - Impact on the task.
    4. Notify the USER (directly in-session or via Lead relay in the active conversation artifact) that a coordination check is required.
...
- **Action**:
    1. Update the lead-to-analyst conversation file with a `## REQUIREMENT FEEDBACK` section.
...
2. **Re-delegate or Refine**:
    - If it's a missing dependency: The Lead may delegate a quick fix to the responsible sub-agent OR ask the current agent to fix it ONLY IF they update the documentation to reflect the fix.
